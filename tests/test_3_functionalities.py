...
def test_x_px_handling():
    ...
    for label,signal in example_signals.items():
        ...
        # x-px lines
        # Take more here, since some might repeat
        spectrum_x_px = nafflib.harmonics(x,px,num_harmonics = 2*n_harm,window_order = 2,window_type = 'hann')
        r_x_px,_,_ = nafflib.find_linear_combinations(spectrum_x_px[1],fundamental_tunes= [spectrum_x_px[1][0]])
...
def test_signal_generation():
    ...
    for (label,signal),tol in zip(example_signals.items(),[1e-12,1e-9,1e-4]):
        ...
        r,_,_  = nafflib.find_linear_combinations(Q,fundamental_tunes= [Q[0]],max_harmonic_order=30)
        ...
        assert np.allclose(x,x_r,atol=tol,rtol=0), f'X-Phasor tracking exceeded tolerance in generate_signal for particle@{label}'
        assert np.allclose(x,x_k,atol=tol,rtol=0), f'X-Phasor tracking exceeded tolerance in generate_pure_KAM for particle@{label}'
        assert np.allclose(px,px_r,atol=tol,rtol=0), f'PX-Phasor tracking exceeded tolerance in generate_signal for particle@{label}'
        assert np.allclose(px,px_k,atol=tol,rtol=0), f'PX-Phasor tracking exceeded tolerance in generate_pure_KAM for particle@{label}'
...
def test_linear_combinations():
    ...
    for (label,signal) in example_signals.items():
        ...
        n_harm = 10
        A = np.exp(-np.array(range(n_harm)))**2 + 1j* (np.pi/np.arange(1,n_harm+1)) *np.exp(-np.array(range(n_harm)))**2
        r = [(1,0,0),
            (2,0,-1),
            (-2,1,0),
            (2,-1,0),
        ...
        x_k,px_k,frequencies = nafflib.generate_pure_KAM(A,r,[Q0,Q1],N,return_frequencies=True)
        ...
        r_found,err,_  = nafflib.find_linear_combinations(frequencies,fundamental_tunes=[Q0,Q1],max_harmonic_order=10)

        assert np.all(np.array(r)==np.array(r_found)), f"Linear combinations don't match for particle@{label}"
        assert np.all(err<tol*10), f"Frequencies found don't match for particle@{label}"
