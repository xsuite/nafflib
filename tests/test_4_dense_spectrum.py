...
def test_dense_spectrum():
    ...
    for num,label in zip([1,2,3],['param_1','param_2','param_3']):
        ...
        # Comparing results
        assert np.allclose(errors_Q,0,atol=1e-9,rtol=0), f'Frequency mismatch for param@{label}'
        assert np.allclose(errors_A,0,atol=1e-5,rtol=0), f'Amplitude mismatch for param@{label}'