...
for data in example_signals:
    ...
    for plane, ax in zip(["x", "y", "zeta"], axes):
        ...
        plt.plot(
            z_rr[N_start:N_stop], pz_rr[N_start:N_stop], ".", color="C0", alpha=0.9
        )

        # If one wants to see the harmonics:
        combin, err, freq = nafflib.find_linear_combinations(
            Q_r, fundamental_tunes=Q_vec
        )
        # print(combin)
...
for plane, ax in zip(["x", "y", "zeta"], axes):
    ...
    plt.plot(
        ...
        color="C0",
        alpha=0.9,
        label=f"nafflib ({num_harmonics} harmonics)",
    )
    plt.legend()
