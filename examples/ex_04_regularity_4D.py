...
look_at = {"x": [(1, 0, 0), (3, 0, -1)], "y": [(0, 1, 0), (1, -1, 0)]}
Q_ref = {"x": [0, 0], "y": [0, 0]}
...
for x0, px0 in zip(x_points, px_point):
    ...
    for plane, ax in zip(["x", "y"], axes):
        ...
        Q_vec = [
            nafflib.tune(dct[f"{plane}"], dct[f"p{plane}"]) for plane in ["x", "y"]
        ]
...
for plane, ax in zip(["x", "y"], axes):
    plt.sca(ax)
    plt.plot(
        np.nan, np.nan, ".", color="k", alpha=0.9, label=f"Line {look_at[plane][0]}"
    )
    plt.plot(
        np.nan, np.nan, ".", color="C0", alpha=0.9, label=f"Line {look_at[plane][1]}"
    )
...
for plane, ax in zip(["x", "y"], axes):
    plt.sca(ax)
    plt.plot(
        np.nan, np.nan, ".", color="k", alpha=0.9, label=f"{look_at[plane][0]}th Line"
    )
    plt.plot(
        np.nan, np.nan, ".", color="C0", alpha=0.9, label=f"{look_at[plane][1]}th Line"
    )
