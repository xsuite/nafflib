...
for x0, px0, color in zip(x_points, px_point, ["C0", "C3", "C4"]):
    ...
    x_e, px_e = nafflib.generate_signal(
        A, Q, np.arange(num_turns - show_turns, num_turns)
    )
...
plt.sca(axes[1])
plt.axvline(
    show_turns, color="k", alpha=0.4, label=f"First {show_turns} | Last {show_turns}"
)
plt.xticks(
    np.arange(0, 2 * show_turns + 30, 30),
    list(np.arange(0, show_turns, 30)) + list(-np.arange(0, show_turns, 30))[::-1],
)
